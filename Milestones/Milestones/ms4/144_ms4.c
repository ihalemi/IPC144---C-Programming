...
// Prints two formats of an items depending on whether linear is true or false
void displayItem(struct Item item,int linear){
  if(linear == 1){
    ...
    // CG2017-08-08: NEED TO ADD A NEWLINE HERE!!!
    puts("");
  }
  else{
    printf("        SKU: %d\n", item.sku);
...
int loadItem(struct Item* item, FILE* dataFile) {
    ...
    // CG2017-08-08: NO NO NO NO NO!!! MULTIPLE RETURNS???? AND USING A FILE POINTER WITHOUT CHECKING IT'S STATE????
    ...
    if (dataFile != NULL)
    {
        // CG2017-08-08: YOU HAD THE 5TH FIELD AS %S (isTaxed part) AND THE DATA FILE HAS AN INTEGER IN THAT POSITION!!!
        //  - ALSO, PROTECT THE NAME MEMBER TO 20 CHARACTERS SINCE IT IS SIZED TO ONLY ACCOMMODATE 20 MEANINGFUL CHARS!
        if (fscanf(dataFile, "%d,%d,%d,%lf,%c,%20[^\n]",
            &item->sku, &item->quantity, &item->minQuantity, &item->price, &item->isTaxed, item->name) == 6)
            retValue = 1;
    }
...
int loadItems(struct Item item[], char filename[], int* NoOfRecsPtr) {
	FILE* fp;
	//int i = 0; // CG2017-08-08: DON'T NEED THIS... USE POINTER PARAM DIRECTLY
	int retValue = 0;

    // CG207-08-08: INITIALIZE THE POINTER PARAM TO ZERO SINCE WE ARE RELOADING ALL ITEMS
    *NoOfRecsPtr = 0;

    struct Item tmpItem;  // CG2017-08-08:  NEED A TEMP. ITEM FOR READING

	fp = fopen(filename, "r");

	//if (fp == NULL) {
	//	return retValue; // CG2017-08-08: STOP THIS NOW... NO MULTPLE RETURNS IN A FUNCTION .. .THERE SHOULD ONLY EVER BE ONE AND ONLY ONE!!!
	//}
    if( fp!= NULL){
        // CG2017-08-08: WOW... THIS IS NOT RIGHT!!! USE A WHILE LOOP!!!
        //for (; loadItem(item, fp) != 0; item++) {
        while( loadItem(&tmpItem, fp) == 1){ // CG2017-08-08:
            // CG2017-08-08: SUCCESSFULLY READ A RECORD INTO THE TMP ITEM... NOW ASSIGN IT TO THE ARRAY:
            item[*NoOfRecsPtr] = tmpItem;
            // CG2017-08-08: INCREMENT THE COUNTER
            (*NoOfRecsPtr) += 1;
			//i += 1;

            retValue = 1;  // CG2017-08-08:  SET THIS TO ONE WHEN YOU HAVE SUCCESSFULLY READ A RECORD
		}
		//*NoOfRecsPtr = i;
		fclose(fp);
		//retValue = 1; // CG2017-08-08: CAN'T MAKE THIS ASSUMPTION JUST BECAUSE THE FILE OPENED THAT YOU HAVE RECORDS...
		
        //return retValue; // CG2017-08-08: NO NO NO... ONLY RETURN ONCE AT THE END OF THE FUNCTION!!!
	}
